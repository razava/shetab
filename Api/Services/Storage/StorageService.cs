...
{
    private readonly IStaticSettings _staticSettings;
    private readonly string destinationPath;
    private readonly List<Size> imageQualities;
    private readonly long maxAllowdFileSize = 10 * 1024 * 1024;

    public StorageService(IWebHostEnvironment webHostEnvironment, IStaticSettings staticSettings)
    {
        _staticSettings = staticSettings;
        destinationPath = webHostEnvironment.WebRootPath;
        imageQualities = _staticSettings.AppSettings.ImageQualityOptions?.ImageQualities ?? new List<Size>() { new Size(100, 100), new Size(200, 200), new Size(300, 300) };
    }
    ...
    public async Task<Media?> WriteFileAsync(IFormFile file, AttachmentType attachmentType)
    {
        ...
        try
        {
            ...
            var pathBuilt = Path.Combine(destinationPath, relativePath);
            ...
            else
            {
                ...
                result = await writeImage(image, destinationPath, relativePath, fileName, imageQualities);
            }
